...
describe("useSafeEffect", () => {
  ...
  it("works with object as extra dep", async () => {
    const sideEffect = jest.fn();
    const C = ({ p1, p2 }: {| p1: { text: string }, p2: number |}) => {
  ...
  it("works with array as extra dep", async () => {
    const countTrue = jest.fn(
      (arr: Array<boolean>): number => arr.filter((x) => x === true).length
    );
  ...
  it("works with combination of object, arrays, and primitives", async () => {
    ...
    await act(async () => {
      render(
        <C p1={{ text: "test_foo" }} p2={["a", "b"]} p3={3} p4 />,
        container
      );
    });
    ...
    await act(async () => {
      render(
        <C p1={{ text: "test_foo" }} p2={["b", "a"]} p3={3} p4 />,
        container
      );
    });
    ...
    await act(async () => {
      render(
        <C p1={{ text: "test_foo" }} p2={["b", "a"]} p3={4} p4 />,
        container
      );
    });
    ...
    await act(async () => {
      render(
        <C p1={{ text: "test_foo" }} p2={["b", "a"]} p3={4} p4={false} />,
        container
      );
    });
